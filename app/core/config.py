...
class WebApiSettings(BaseSettings):
    ...
    login_start_payload: str = Field(
        default="",
        description="Telegram /start payload used to request a web-admin magic link.",
    )
    ...
    @field_validator("login_start_payload")
    @classmethod
    def validate_login_start_payload(cls, v: str) -> str:
        payload = v.strip()
        if not payload:
            return ""
        if len(payload) > 64 or re.fullmatch(r"[A-Za-z0-9_-]+", payload) is None:
            raise ValueError("login_start_payload must be 1-64 characters of A-Z, a-z, 0-9, _ or -")
        if payload == "ads" or re.fullmatch(r"adlead_\d+", payload) is not None:
            raise ValueError("login_start_payload must not collide with existing public bot deep links")
        return payload

    @model_validator(mode="after")
    def validate_magic_link_payload(self) -> WebApiSettings:
        if self.auth_mode == "magic_link" and not self.login_start_payload:
            raise ValueError("login_start_payload is required when auth_mode=magic_link")
        return self
